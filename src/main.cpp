...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Raven: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Raven Signed Message:\n";
...
{
    // Raven: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Raven: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Raven: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Raven: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Raven: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Raven: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Raven: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "addr")
    {
        ...
        // Don't want addr from older versions unless RVNing
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RavenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RavenMiner : ProcessBlock, block not accepted");
    }
...
void static RavenMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running RavenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
