...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Raven version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  Ravend [options]                     " + "\n" +
                  "  Ravend [options] <command> [params]  " + _("Send command to -server or Ravend") + "\n" +
                  "  Ravend [options] help                " + _("List commands") + "\n" +
                  "  Ravend [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Raven:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: Raven.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: Ravend.pid)") + "\n" +
        ...
        "  -dns                   " + _("Allow DNS lookups for -addnode, -RVNnode and -connect") + "\n" +
        "  -port=<port>           " + _("Listen for connections on <port> (default: 9333 or testnet: 19333)") + "\n" +
        "  -maxconnections=<n>    " + _("Maintain at most <n> connections to peers (default: 125)") + "\n" +
        "  -addnode=<ip>          " + _("Add a node to connect to and attempt to keep the connection open") + "\n" +
        "  -connect=<ip>          " + _("Connect only to the specified node(s)") + "\n" +
        "  -RVNnode=<ip>         " + _("Connect to a node to retrieve peer addresses, and disconnect") + "\n" +
        ...
        "  -dnsRVN               " + _("Find peers using DNS lookup (default: 1 unless -connect)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Raven Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-connect") && mapMultiArgs["-connect"].size() > 0) {
        // when only connecting to trusted nodes, do not RVN via DNS, or listen by default
        SoftSetBoolArg("-dnsRVN", false);
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Raven is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Raven version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    BOOST_FOREACH(string strDest, mapMultiArgs["-RVNnode"])
        AddOneShot(strDest);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Raven") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Raven to complete") << "\n";
