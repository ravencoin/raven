...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Raven " + FormatFullVersion();
...
// DNS RVNs
// Each pair gives a source name and a RVN name.
// The first name is used as information source for addrman.
// The second name should resolve to a list of RVN addresses.
static const char *strMainNetDNSRVN[][2] = {
    {"Raventools.com", "dnsRVN.Raventools.com"},
    {"Ravenpool.org", "dnsRVN.Ravenpool.org"},
    {"xurious.com", "dnsRVN.RVN.xurious.com"},
    {"koin-project.com", "dnsRVN.koin-project.com"},
    {"weminemnc.com", "dnsRVN.weminemnc.com"},
    {NULL, NULL}
};

static const char *strTestNetDNSRVN[][2] = {
    {"Raventools.com", "testnet-RVN.Raventools.com"},
    {"xurious.com", "testnet-RVN.RVN.xurious.com"},
    {"wemine-testnet.com", "dnsRVN.wemine-testnet.com"},
    {NULL, NULL}
};

void ThreadDNSAddressRVN()
{
    static const char *(*strDNSRVN)[2] = fTestNet ? strTestNetDNSRVN : strMainNetDNSRVN;

    int found = 0;

    printf("Loading addresses from DNS RVNs (could take a while)\n");

    for (unsigned int RVN_idx = 0; strDNSRVN[RVN_idx][0] != NULL; RVN_idx++) {
        if (HaveNameProxy()) {
            AddOneShot(strDNSRVN[RVN_idx][1]);
        } else {
            vector<CNetAddr> vaddr;
            vector<CAddress> vAdd;
            if (LookupHost(strDNSRVN[RVN_idx][1], vaddr))
            {
            ...
            addrman.Add(vAdd, CNetAddr(strDNSRVN[RVN_idx][0], true));
        }
    }

    printf("%d addresses found from DNS RVNs\n", found);
}
...
void ThreadOpenConnections()
{
    ...
    loop
    {
        ...
        // Add RVN nodes if IRC isn't working
        if (addrman.size()==0 && (GetTime() - nStart > 60) && !fTestNet)
        {
            std::vector<CAddress> vAdd;
            for (unsigned int i = 0; i < ARRAYLEN(pnRVN); i++)
            {
                // It'll only connect to one or two RVN nodes because once it connects,
                // it'll get a pile of addresses with newer timestamps.
                // RVN nodes are given a random 'last seen time' of between one and two
                // weeks ago.
                const int64 nOneWeek = 7*24*60*60;
                struct in_addr ip;
                memcpy(&ip, &pnRVN[i], sizeof(ip));
        ...
        loop
        {
            ...
            // stop this loop, and let the outer loop run again (which sleeps, adds RVN nodes, recalculates
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Raven is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    ...
    if (!GetBoolArg("-dnsRVN", true))
        printf("DNS RVNing disabled\n");
    else
        threadGroup.create_thread(boost::bind(&TraceThread<boost::function<void()> >, "dnsRVN", &ThreadDNSAddressRVN));
